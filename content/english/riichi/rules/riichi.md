...
**Riichi** {{< kana "立直" >}} or {{< kana "リーチ" >}} is the most commonly used
[yaku]({{< ref "/riichi/yaku/list-of-yaku.md" >}}) in the game. Among the yaku, riichi occurs at
least 40% of the time across different mahjong platforms and professional settings. This yaku may
apply to any closed hand and [may be declared]({{< ref "/riichi/rules/naki.md" >}}) upon achieving
[tenpai]({{< ref "/riichi/strategy/tenpai.md" >}}). As such, riichi is attainable via numerous hand
configurations, as long as it is closed. Due to its common usage, various specific game scenarios
must be considered when using riichi, or facing [opponents who have called
riichi]({{< ref "/riichi/strategy/defense.md" >}}).
...
status, using a tactic known as [damaten]({{< ref "/riichi/strategy/damaten.md" >}}). The decision
for using riichi rests on some factors, primarily on need or desire. Current point standings often
factor towards a riichi call, especially when falling behind in points. On the other hand, point
leaders may tend to be conservative to limit opportunities for other players to score hands
directly.
...
may involve calls for [kan]({{< ref "/riichi/rules/kan.md" >}}).
...
to call riichi is when the player has a chance to score riichi,
[haitei]({{< ref "/riichi/yaku/haitei-raoyue-and-houtei-raoyui.md" >}}),
[ippatsu]({{< ref "/riichi/yaku/optional/ippatsu.md" >}}), and
[mentsumo]({{< ref "/riichi/yaku/menzenchin-tsumohou.md" >}}).

### Noten riichi

A player may actually declare riichi with a [noten hand]({{< ref "/riichi/strategy/tenpai.md" >}}).
This is a **noten riichi** {{< kana "ノテン立直" >}}, which is highly not advisable. Of course,
computer based play is usually programmed to prevent players from making such a riichi declaration.
If and when the hand results in [ryuukyoku]({{< ref "/riichi/rules/ryuukyoku.md" >}}), then it is
required of the player to reveal the hand to show tenpai. The noten riichi would naturally show
noten; and it is subject to [chombo]({{< ref "/riichi/rules/chombo.md" >}}). Likewise, chombo is
applied if a win is called, as the hand's winning status would be verified only to reveal an invalid
hand. Players may escape this dubious position, when another player wins the hand or has committed a
chombo penalty. An [abortive draw]({{< ref "/riichi/rules/tochuu-ryuukyoku.md" >}}) may also prevent
the chombo penalty, except suucha riichi.
...
tiles]({{< ref "/riichi/strategy/betaori.md" >}}). While it is advised to declare a win on the first
opportunity, some plays may require a player not to do so under very specific circumstances. Of
course, such play requires caution.
...
When a riichi declarer wins, he may flip the tiles underneath the
[dora]({{< ref "/riichi/rules/dora.md" >}}) and kan dora indicators. These flipped _ura dora
indicators_ may increase the value of his hand.
...
**Open riichi** {{< kana "オープン立直" >}} is an [optional
yaku]({{< ref "/riichi/yaku/optional/local-yaku.md" >}}), which is a modified version to riichi
itself. The principle works exactly in the same way as the original riichi. However, there is an
added bonus of 1-han attached, which is earned by revealing either the hand or the tile waits. This
naturally eases everyone else's ability to defend against the riichi. In general, the intent
...
for all players to have declared riichi via [suucha
riichi]({{< ref "/riichi/rules/tochuu-ryuukyoku.md" >}}).

### Karaten riichi

Riichi may actually be called if all waiting tiles are rendered unavailable. A hand in this state is
rendered as [karaten]({{< ref "/riichi/terminology/karaten.md" >}}). In this state, the hand is not
winnable as the number of available winning tiles is zero. Such calls may be made by mistake, or
intentionally to cause players to defend.
...
discard. [Tile calls]({{< ref "/riichi/rules/naki.md" >}}), including
[kan]({{< ref "/riichi/rules/kan.md" >}}), are able to nullify ippatsu by denying it.
...
turn order before the declaration, where applicable. For the
[dealer]({{< ref "/riichi/strategy/jikaze.md" >}}), double riichi can never be interrupted. As the
name implies, this yaku is worth 2 han instead of 1 han, as a bonus for the initial timing. With the
added han value, a call for double riichi has a huge advantage over the other players in conjunction
with the immediate speed and value compared to conventional riichi calls.
...
Once again, the prospect for [rinshan kaihou]({{< ref "/riichi/yaku/rinshan-kaihou.md" >}}) to work
with riichi is dependent on the legality of the kan call during riichi. With computer interfaces,
this rule may be enforced by disabling a player's ability to call the kan during this situation.
However, with actual tiles, a player must be aware of this legality. Otherwise, chombo may be
enforced instead.
...
riichi at all and instead employ the strategy of
[damaten]({{< ref "/riichi/strategy/damaten.md" >}}).
