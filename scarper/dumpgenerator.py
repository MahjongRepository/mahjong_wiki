...
def getNamespacesScraper(config={}, session=None):
    ...
    if namespaces:
        r = session.post(url=config["index"], params={"title": "Special:Allpages"}, timeout=30)
        raw = r.text
        ...
        while r_suballpages and re.search(r_suballpages, raw) and c < deep:
            ...
            for i in m:
                ...
                if name not in checked_suballpages:
                    ...
                    print "    Reading", name, len(raw2), "bytes", len(
                        re.findall(r_suballpages, raw2)
                    ), "subpages", len(re.findall(r_title, raw2)), "pages"
...
def getPageTitles(config={}, session=None):
    ...
    titlesfilename = "%s-titles.txt" % domain2prefix(config=config)
...
def getXMLHeader(config={}, session=None):
    ...
    if config["xmlrevisions"] and config["api"] and config["api"].endswith("api.php"):
        try:
            ...
            r = requests.get(
                config["api"] + "?action=query&export=1&exportnowrap=1&list=allpages&aplimit=1", timeout=10
            )
            xml = r.text
            # Otherwise try without exportnowrap, e.g. Wikia returns a blank page on 1.19
            if not re.match(r"\s*<mediawiki", xml):
                r = requests.get(
                    config["api"] + "?action=query&export=1&list=allpages&aplimit=1&format=json", timeout=10
                )
                try:
            ...
            if not re.match(r"\s*<mediawiki", xml):
                ...
                r = requests.get(
                    config["api"] + "?action=query&export=1&exportnowrap=1&titles=" + randomtitle, timeout=10
                )
                xml = r.text
            # Again try without exportnowrap
            if not re.match(r"\s*<mediawiki", xml):
                r = requests.get(
                    config["api"] + "?action=query&export=1&format=json&titles=" + randomtitle, timeout=10
                )
                try:
                    xml = r.json()["query"]["export"]["*"]
                except KeyError:
    ...
    while not re.search(r"</mediawiki>", xml):
        ...
        if c >= maxretries:
            ...
            if not config["curonly"] and not "curonly" in params:
                ...
                logerror(
                    config=config,
                    text=u'Error while retrieving the last revision of "%s". Skipping.' % (params["pages"]),
                )
...
def generateXMLDump(config={}, titles=[], start=None, session=None):
    ...
    header, config = getXMLHeader(config=config, session=session)
    footer = "</mediawiki>\n"  # new line at the end
    xmlfilename = "%s.xml" % domain2prefix(config=config)
    xmlfile = ""
    lock = True
...
def getXMLRevisions(config={}, session=None, allpages=False, start=None):
    ...
    try:
        for namespace in namespaces:
            ...
            arvparams = {"action": "query", "list": "allrevisions", "arvlimit": 50, "arvnamespace": namespace}
...
def makeXmlFromPage(page):
    ...
    try:
        ...
        for rev in page["revisions"]:
            ...
            if "user" in rev:
                revision.append(E.contributor(E.username(to_unicode(rev["user"])), E.id(to_unicode(userid)),))
            else:
...
def saveImageNames(config={}, images=[], session=None):
    ...
    imagesfilename = "%s-images.txt" % domain2prefix(config=config)
    imagesfile = open("%s/%s" % (config["path"], imagesfilename), "w")
    imagesfile.write(
        (
            "\n".join(
                ["%s\t%s\t%s" % (filename, url, uploader) for filename, url, uploader in images]
            ).encode("utf-8")
        )
    )
    imagesfile.write("\n--END--")
...
def getImageNamesScraper(config={}, session=None):
    ...
    while offset:
        ...
        # delicate wiki
        if re.search(
            ur"(?i)(allowed memory size of \d+ bytes exhausted|Call to a member function getURL)", raw
        ):
            if limit > 10:
        ...
        if "query" in jsonimages:
            ...
            for image in jsonimages["query"]["allimages"]:
                ...
                if "api" in config and (".wikia." in config["api"] or ".fandom.com" in config["api"]):
                    ...
                    filename = unicode(
                        urllib.unquote((re.sub("_", " ", url.split("/")[-3])).encode("ascii", "ignore")),
                        "utf-8",
                    )
                else:
                    filename = unicode(
                        urllib.unquote((re.sub("_", " ", url.split("/")[-1])).encode("ascii", "ignore")),
                        "utf-8",
                    )
...
def getParameters(params=[]):
    ...
    parser.add_argument("--delay", metavar=5, default=0, type=float, help="adds a delay (in seconds)")
    ...
    parser.add_argument(
        "--resume", action="store_true", help="resumes previous incomplete dump (requires --path)"
    )
    ...
    groupDownload.add_argument("--images", action="store_true", help="generates an image dump")
    groupDownload.add_argument(
        "--namespaces",
        metavar="1,2,3",
    ...
    groupDownload.add_argument(
        "--exnamespaces", metavar="1,2,3", help="comma-separated value of namespaces to exclude"
    )

    # Meta info params
    groupMeta = parser.add_argument_group("Meta info", "What meta info to retrieve from the wiki")
    groupMeta.add_argument("--get-wiki-engine", action="store_true", help="returns the wiki engine")
    ...
    try:
        ...
        __retries__ = Retry(total=5, backoff_factor=2, status_forcelist=[500, 502, 503, 504])
...
def checkAPI(api=None, session=None):
    ...
    for i in range(4):
        ...
        r = session.get(url=api, params={"action": "query", "meta": "siteinfo", "format": "json"}, timeout=30)
    ...
    try:
        ...
        if result:
            try:
                index = result["query"]["general"]["server"] + result["query"]["general"]["script"]
...
def checkIndex(index=None, cookies=None, session=None):
    ...
    if (
        ...
        return False
    if re.search(
        r'(page-Index_php|"wgPageName":"Index.php"|"firstHeading"><span dir="auto">Index.php</span>)', raw
    ):
...
def resumePreviousDump(config={}, other={}):
    ...
    if config["images"]:
        ...
        try:
            f = open(
                "%s/%s-%s-images.txt" % (config["path"], domain2prefix(config=config), config["date"]), "r"
            )
...
def saveSpecialVersion(config={}, session=None):
    ...
    if os.path.exists("%s/Special:Version.html" % (config["path"])):
        ...
        r = session.post(url=config["index"], params={"title": "Special:Version"}, timeout=10)
        raw = r.text
        delay(config=config, session=session)
...
def getWikiEngine(url=""):
    ...
    if re.search(ur'(?im)(<meta name="generator" content="DokuWiki)|dokuwiki__site', result):
        ...
        ur'(?im)(<meta name="generator" content="Tiki Wiki|Powered by <a href="http://(www\.)?tiki\.org"| id="tiki-(top|main)")',
        result,
    ):
        wikiengine = "TikiWiki"
        ...
        ur'(?im)(id="xwiki(content|nav_footer|platformversion|docinfo|maincontainer|data)|/resources/js/xwiki/xwiki|XWiki\.webapppath)',
        result,
    ):
        wikiengine = "XWiki"
        ...
        ur'(?im)(Wheeled by <a class="external-link" href="http://www\.wagn\.org">|<body id="wagn">)', result
    ):
        wikiengine = "Wagn"
    elif re.search(ur'(?im)(<meta name="generator" content="MindTouch)', result):
        wikiengine = "MindTouch"  # formerly DekiWiki
    elif re.search(
        ur'(?im)(<div class="wikiversion">\s*(<p>)?JSPWiki|xmlns:jspwiki="http://www\.jspwiki\.org")', result
    ):
        wikiengine = "JSPWiki"
    elif re.search(
        ur'(?im)(Powered by:?\s*(<br ?/>)?\s*<a href="http://kwiki\.org">|\bKwikiNavigation\b)', result
    ):
        wikiengine = "Kwiki"
    elif re.search(ur'(?im)(Powered by <a href="http://www\.anwiki\.com")', result):
        wikiengine = "Anwiki"
    elif re.search(
        ur'(?im)(<meta name="generator" content="Aneuch|is powered by <em>Aneuch</em>|<!-- start of Aneuch markup -->)',
        result,
    ):
        wikiengine = "Aneuch"
    ...
    # WakkaWiki forks
        ...
        ur'(?im)(<meta name="generator" content="WikkaWiki|<a class="ext" href="(http://wikka\.jsnx\.com/|http://wikkawiki\.org/)">)',
        result,
    ):
    ...
    # Custom wikis used by wiki farms
        ...
        ur'(?im)(Powered by <a href="http://www\.wikidot\.com">|wikidot-privacy-button-hovertip|javascript:WIKIDOT\.page)',
        result,
    ):
        wikiengine = "Wikidot"
